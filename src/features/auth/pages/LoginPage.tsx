...
export function LoginPage() {
  ...
  const loginMutation = useMutation({
    ...
    onError: (error: { message?: string; error?: string; code?: string; userId?: string }) => {
      if (error.code === "EMAIL_NOT_VERIFIED" && error.userId) {
        toast.error("Please verify your email first");
        navigate({ to: "/verify", search: { userId: error.userId } });
        return;
      }
  ...
  const isDisabled = loginMutation.isPending || loginMutation.isSuccess;
          ...
          <form onSubmit={onSubmit} className="space-y-5" id="login-form">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  autoComplete="email"
                  disabled={isDisabled}
                  className="w-full rounded-xl border border-border bg-input px-4 py-3 pl-11 text-sm font-medium text-foreground placeholder:text-muted-foreground outline-none transition-all duration-200 hover:border-[oklch(1_0_0/15%)] focus:border-violet/40 focus:ring-2 focus:ring-ring disabled:opacity-50"
                  {...register("email")}
                />
              </div>
              {errors.email && (
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <Link
                  to="/forgot-password"
                  className="text-xs font-medium text-violet-light hover:underline cursor-pointer"
                >
                  Forgot password?
                </Link>
              </div>
              <div className="relative">
                ...
                <input
                  ...
                  disabled={isDisabled}
                  className="w-full rounded-xl border border-border bg-input px-4 py-3 pl-11 pr-11 text-sm font-medium text-foreground placeholder:text-muted-foreground outline-none transition-all duration-200 hover:border-[oklch(1_0_0/15%)] focus:border-violet/40 focus:ring-2 focus:ring-ring disabled:opacity-50"
                  {...register("password")}
                />
                <button
                  type="button"
