...
describe("ApiClient (apiClient)", () => {
  ...
  describe("401 Interceptor", () => {
    it("should refresh token and retry request on 401", async () => {
      ...
      // 1. Original request
      expect(fetchMock).toHaveBeenNthCalledWith(1, expect.stringContaining("/protected"), expect.anything());
      // 2. Refresh request
      expect(fetchMock).toHaveBeenNthCalledWith(2, expect.stringContaining("/api/auth/refresh"), expect.anything());
      // 3. Retry request
      expect(fetchMock).toHaveBeenNthCalledWith(3, expect.stringContaining("/protected"), expect.anything());
    ...
    it("should logout if refresh fails", async () => {
      ...
      const fetchMock = vi.fn()
        ...
        .mockResolvedValue({ ok: false, status: 401, json: () => Promise.resolve({ message: "Unauthorized", code: "UNAUTHORIZED", status: 401 }) }); // Retry (should not happen but just in case)
  ...
  describe("Standard Request Methods", () => {
    ...
    it("PUT should function correctly", async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ method: "PUT" }));
    });
    ...
    it("PATCH should function correctly", async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ method: "PATCH" }));
    });
    ...
    it("DELETE should function correctly", async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ method: "DELETE" }));
    });
